...
class ManageYAML:
    ...
    def __init__(self):
        ...
        self.folder = rospy.get_param('~folder', os.path.join(
            rospack.get_path('poi_manager'), 'config'))
        self.yaml_path = self.folder + '/' + self.filename+'.yaml'

        self.pose_list = []
        self.pose_dict = {}
        self.service_read_yaml = rospy.Service(
            '~read_pois', ReadPOIs, self.handle_labeled_pose_list)
        self.service_write_data = rospy.Service(
            '~update_pois', UpdatePOIs, self.handle_updated_list)
        ...
        rospy.loginfo('%s::_init_: config file path: %s',
                      rospy.get_name(), self.yaml_path)
    ...
    def manage_read_data(self):
        ...
        for key, value in self.pose_dict.items():
            self.pose_list.append(LabeledPose(
                key, Pose2D(value[0], value[1], value[2])))
    ...
    def update_yaml(self, req):
        ...
        for elem in pose_list:
            self.pose_dict[elem.label] = [
                elem.pose.x, elem.pose.y, elem.pose.theta]
    ...
    def handle_labeled_pose_list(self, req):
        ...
        rospy.loginfo(
            "%s::handle_labeled_pose_list: read_pois service done", rospy.get_name())
        return ReadPOIsResponse(self.pose_list)

    def handle_updated_list(self, req):
        self.update_yaml(req)
        rospy.loginfo(
            "%s::handle_updated_list: update_pois service done", rospy.get_name())
